...
Cells are [first-class](https://en.wikipedia.org/wiki/First-class_citizen) [reference types](https://en.wikipedia.org/wiki/Value_type_and_reference_type) that are [passed by value](https://en.wikipedia.org/wiki/Evaluation_strategy#Call_by_value). They are implemented as [persistent data structures](https://en.wikipedia.org/wiki/Persistent_data_structure#Persistent_hash_array_mapped_trie), an "[observer](https://en.wikipedia.org/wiki/Observer_(quantum_physics))" of a cell gets a [snapshot](https://en.wikipedia.org/wiki/Snapshot_(computer_storage)) of the cell and its state at that instant in time. [Mutating](https://en.wikipedia.org/wiki/Mutation) a cell creates a new local variant based on that snapshot, with structural sharing of its past variants.
