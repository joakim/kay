...
Cells are by default [encapsulated and opaque](https://en.wikipedia.org/wiki/Information_hiding). The internal state may only be accessed from the outside through setter and getter messages, unless slots are explicitly exposed. Exceptions are handled internally, a cell can not crash.
